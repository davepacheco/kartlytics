...
#
# run_videos.sh: runs the whole kartlytics pipeline on selected videos
#
...
while read filename; do
	echo "$filename" >> $ra_tmpfile
done

echo "Working on videos:"
cat $ra_tmpfile

echo "Running job to process videos:"
cat $ra_tmpfile | mjob create -w \
